...
class ICM20948:
    ...
    def __init__(self, i2c_addr=0x69, i2c_bus=1, accel_range=2, gyro_range=500, config_file = None):
    ...
    def load_config(self, config_file):
        ...
        try:
            ...
            self._accel_LPF_CFG = config[section].getint('accel_LPF_CFG', self._accel_LPF_CFG)
            self._gyro_LPF = config[section].getboolean('gyro_LPF', self._gyro_LPF)
            self._gyro_LPF_CFG = config[section].getint('gyro_LPF_CFG', self._gyro_LPF_CFG)
            
            section = 'Calibration'
            # Parse accelerometer bias
            accel_bias_str = config[section].get('accel_bias', ','.join(map(str, self._accel_bias)))
            self._accel_bias = [float(x) for x in accel_bias_str.split(',')]

            # Parse gyroscope bias
            gyro_bias_str = config[section].get('gyro_bias', ','.join(map(str, self._gyro_bias)))
    ...
    def select_register_bank(self, bank):
        ...
        if bank != self._bank:
            ...
            logger.debug(f"Register Bank Changed to: 0x{self.read(self.reg.REG_BANK_SEL):02X}")  
...
if __name__ == "__main__":
    try:
        ...
        while True:
            accel_x, accel_y, accel_z, gx, gy, gz = imu.read_accelerometer_gyro(convert = True)                              

            print(f"Accel|>  X: {accel_x:.6f}, Y: {accel_y:.6f}, Z: {accel_z:.6f}, <Gyro|> X: {gx:.6f}, Y: {gy:.6f}, Z: {gz:.6f}")
