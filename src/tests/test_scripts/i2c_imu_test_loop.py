...
async def control_loop(imu, num_iterations=100):
    ...
    imu_data_array = np.zeros(
        (6, num_iterations * N_READS)
    )  # 6 rows for ax, ay, az, gx, gy, gz
    ...
    for i in range(num_iterations):
        ...
        for j in range(N_READS):
            # Read IMU data
            imu_data_array[:, i * N_READS + j] = imu.read_accelerometer_gyro(
                convert=True
            )  # Get IMU data
...
def main():
    ...
    start_times, imu_data = loop.run_until_complete(
        control_loop(imu, num_iterations=num_iterations)
    )
